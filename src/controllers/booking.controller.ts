...
import { createBooking, deleteBooking, getBookingById, getFacilityBookings, getFarmerBookings, updateBookingStatus } from '../services/booking.service';
...
// YET TO FIX
export const listFacilityBookings = async (req: Request, res: Response): Promise<void> => {
...
export const expireBooking = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { bookingId } = req.params;
    const booking = await updateBookingStatus(BigInt(bookingId), BookingStatus.INACTIVE);
